...
class FullTraceDiffAttackPGD:
    ...
    def __init__(self, speaker_model, purification_model, config):
        ...
        self.t_interval = max(1, int(getattr(config, "diffattack_t_interval", 1)))
    ...
    def attack(self, x_clean, return_trajectory=False):
        ...
        trajectory = {"total_loss": [], "direct_loss": [], "purif_loss": [], "mse_loss": []} if return_trajectory else None
        ...
        for i in range(self.num_iterations):
            ...
            if self.weight_strategy == "staged":
                a, b, c = (1.0, 0.0, 0.0) if i < self.stage_switch else (self.alpha, self.beta, self.gamma)
            else:
            ...
            if b != 0.0 or c != 0.0:
                ...
                if c != 0.0:
                    ...
                    mse_terms = [
                        F.mse_loss(mid_x[current_t], fwd_adv[current_t])
                        for current_t in schedule
                        if current_t in fwd_adv
                    ]
                    loss_mse = (
                        torch.stack(mse_terms).mean()
                        if mse_terms
                        else torch.zeros((), device=x_adv.device)
                    )
            ...
            if loss_mse is None:
                loss_mse_for_loss = torch.zeros((), device=x_adv.device)
                loss_mse_log = None
            else:
                loss_mse_for_loss = loss_mse
                loss_mse_log = loss_mse.item()

            loss = a * loss_direct + b * loss_purif_for_loss + c * loss_mse_for_loss
            loss.backward()
            ...
            if i % 10 == 0 or i == self.num_iterations - 1:
                ...
                mse_msg = "skipped" if loss_mse_log is None else f"{loss_mse_log:.4f}"
                print(
                    f"  fulltrace iter {i:02d}: total={loss.item():.4f}  "
                    f"direct={loss_direct.item():.4f}  purif={purif_msg}  mse={mse_msg}",
                    flush=True,
                )
            ...
            if return_trajectory and i % 10 == 0:
                ...
                trajectory["mse_loss"].append(loss_mse_log)

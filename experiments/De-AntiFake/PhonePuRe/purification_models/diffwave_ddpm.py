...
class DiffWave(torch.nn.Module):
    ...
    def _diffusion(self, x_0: Union[torch.Tensor, np.ndarray]) -> torch.Tensor: 
        ...
        x_t = torch.sqrt(Alpha_bar[self.reverse_timestep-1]) * x_0 + torch.sqrt(1-Alpha_bar[self.reverse_timestep-1]) * z
        return x_t
    ...
    def _reverse_step(self, x_t: torch.Tensor, current_t: int, prev_t: int, noise: torch.Tensor) -> torch.Tensor:
        ...
        pred_x0 = (x_t - sqrt_one_minus_alpha_bar_current * epsilon_theta) / sqrt_alpha_bar_current
    ...
    def _reverse(self, x_t: Union[torch.Tensor, np.ndarray]) -> torch.Tensor: 
        ...
        noises = {
            current_t: torch.randn_like(x_t_rev)
        ...
        for idx, current_t in enumerate(schedule):
            ...
            if self.use_checkpoint and x_t_rev.requires_grad:
                x_t_rev = checkpoint(
                    lambda inp, step=current_t, next_step=prev_t, step_noise=noise: self._reverse_step(inp, step, next_step, step_noise),
                    x_t_rev,
                    use_reentrant=False,
                )
            else:
    ...
    def fast_reverse(self, x_t: Union[torch.Tensor, np.ndarray]) -> torch.Tensor: 
        ...
        for t in range(K-1, -1, -1):
            ...
            real_t = S[t]
            eps_St = self.model((x_St, real_t * torch.ones((x_St.shape[0], 1), device=self.device)))

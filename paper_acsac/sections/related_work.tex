...
De-AntiFake directly evaluates this risk for voice cloning and shows that purification can bypass several protective perturbation defenses \cite{fan2025deantifake}. 
...
\caption{\method{} framework and trust boundary. The defender optimizes a bounded protective perturbation through a local surrogate diffusion purifier and encoder ensemble; 
the attacker's target purifier and cloning backend are unseen during optimization and are used only for transfer evaluation.}
...
DiffAttack suggests that output-only supervision can miss behavior inside the diffusion trajectory and proposes intermediate-step supervision \cite{kang2023diffattack}. 
DiffBreak shows that stochastic and adaptive evaluation can overturn apparent robustness claims for diffusion purification \cite{kassis2025diffbreak}. 
...
\method{} borrows the trajectory-aware view from this line of work, but applies it to a different security objective: 
protecting released speech from post-purification voice cloning under perceptual audio-quality constraints and continuous speaker-identity metrics.
